...
_sdcard_widthx_txt=$(mktemp)
_sdcard_heighty_txt=$(mktemp)
...
awk '{sum = $3 - $1; print sum}' "$_sdcard_bounds_txt" >"$_sdcard_widthx_txt"
awk '{sum = $4 - $2; print sum}' "$_sdcard_bounds_txt" >"$_sdcard_heighty_txt"
...
printf "%s\t" "index" "text" "resource-id" "class" "package" "content-desc" "checkable" "checked" "clickable" "enabled" "focusable" "focused" "scrollable" "long-clickable" "password" "selected" "startx" "endx" "starty" "endy" "centerx" "centery" "area" "widthx" "heighty" | echo "$(cat)" > "$_sdcard_u_txt"
paste "$_sdcard_index_txt" "$_sdcard_text_txt" "$_sdcard_resource_id_txt" "$_sdcard_class_txt" "$_sdcard_package_txt" "$_sdcard_content_desc_txt" "$_sdcard_checkable_txt" "$_sdcard_checked_txt" "$_sdcard_clickable_txt" "$_sdcard_enabled_txt" "$_sdcard_focusable_txt" "$_sdcard_focused_txt" "$_sdcard_scrollable_txt" "$_sdcard_long_clickable_txt" "$_sdcard_password_txt" "$_sdcard_selected_txt" "$_sdcard_startx_txt" "$_sdcard_endx_txt" "$_sdcard_starty_txt" "$_sdcard_endy_txt" "$_sdcard_centerx_txt" "$_sdcard_centery_txt" "$_sdcard_area_txt" "$_sdcard_widthx_txt" "$_sdcard_heighty_txt" >> "$_sdcard_u_txt"
...
rm -f "$_sdcard_widthx_txt"
rm -f "$_sdcard_heighty_txt"
