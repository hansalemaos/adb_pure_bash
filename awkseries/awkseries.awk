...
function count_char(v,character) {
    system("echo "v" | xargs > "tmptmptmpdata)
    getline stringx < tmptmptmpdata
    close(tmptmptmpdata)
    countcommand="awk -F"character" '{print NF-1}' <<< "stringx" > "tmptmptmpdata
    system(countcommand)
    getline count < tmptmptmpdata
    close(tmptmptmpdata)
    print count;
}
...
function sort_letters(v){
    system("echo "v" | tr -d '[:space:]' | grep -o . | sort | tr -d \"\\n\" > "tmptmptmpdata)
    getline stringx < tmptmptmpdata
    close(tmptmptmpdata)
    print stringx;
}
function str_replace(v,subs1,subs2){
    system("printf \"%s\" \""v"\" | awk -v subs1=\""subs1"\" -v subs2=\""subs2"\" '{ gsub(subs1, subs2) } 1' > "tmptmptmpdata)
    getline stringx < tmptmptmpdata
    close(tmptmptmpdata)
    print stringx;
...
function str_startswith(v,value){
    system("if [[ "v" == "value"* ]] ; then echo 1; fi > "tmptmptmpdata)
    getline stringx < tmptmptmpdata
    close(tmptmptmpdata)
    if (stringx ~ /^[:space:]*$/){
        stringx=0;
...
function str_endswith(v,vala){
    cmdmktemp = "mktemp"
    if ((cmdmktemp | getline tmptmptmpdata) > 0) {
        close(cmdmktemp)
    }
    system("if [[ "v" == *"vala" ]] ; then echo 1; fi > "tmptmptmpdata)
    getline stringxx < tmptmptmpdata
    close(tmptmptmpdata)
    if (stringxx ~ /^[:space:]*$/){
        stringxx=0;
    }
